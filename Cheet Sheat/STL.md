...
**Core Time Complexity:** Insert/Search (find)/Erase: $O(1)$ average, $O(N)$ worst-case
...
if (visited.count(node))                             // Check if a node was visited, Args: (const Key& key). Returns: size_t (1 if exists, 0 otherwise)
visited.insert(10);                                  // Mark a node as visited, Args: (const T& value). Returns: std::pair<iterator, bool> (bool is true if newly inserted)
...
std::bitset<32> mask(10);                            // a 32 bits mask & initilize value = 10, Args: (unsigned long long val). Returns: Bitset representing 1010
...
### Standard Input Processing (UVa & CP Patterns)

**Core Time Complexity:** $O(1)$ per read operation, total $O(V)$ where $V$ is total input size
...
// 1. Reading Fixed Number of Test Cases
int t;
std::cin >> t;                                        // Args: [variable reference]. Returns: std::istream reference
while (t--) {                                         // Evaluates t, then decrements. Stops when t == 0
    // Process test case
}

// 2. Reading Until End of File (EOF) - UVa Standard
int n;
while (std::cin >> n) {                               // Returns: std::istream reference. Evaluates to false on EOF or failure
    // Process n
}

// 3. Reading Until Termination Condition (e.g., both 0)
int a, b;
while (std::cin >> a >> b && (a != 0 || b != 0)) {    // Chaining streams. Evaluates to false if EOF or a=0, b=0
    // Process a and b
}

// 4. Reading Full Lines with Spaces (std::getline)
int id;
std::cin >> id;

// CRITICAL: std::cin leaves the '\n' character in the buffer. 
// Must ignore it before calling getline, otherwise getline reads an empty string.
std::cin.ignore();                                    // Args: None (defaults to 1 char, delim EOF). Returns: std::istream reference

std::string line;
std::getline(std::cin, line);                         // Args: (std::istream& is, std::string& str). Returns: std::istream reference

// 5. Parsing a Line of Unknown Length (std::stringstream)
// Extremely common in UVa when array size is not given
std::string raw_data = "10 20 30 40";
std::stringstream ss(raw_data);                       // Args: (const std::string& str). Returns: std::stringstream object

int val;
while (ss >> val) {                                   // Returns: std::istream reference. Evaluates to false when tokens exhaust
    // val becomes 10, then 20, then 30, then 40
}

```


1. unordered_map<int, int> freq = {{1,2}, {2,3}}; // use { , }
2. stringstream
3. no constructor => element e{ , };
4. .first .second
5. for (const auto& p : freq) cout << p.first << ": " << p.second << "\n";
6. s.substr(0, count)
7. string::npos
8. stoi("123"), to_string(123)
9. memo.count()
10. sort => binary_search();
11. bool operator<(const element& other) const 
12. can directly use weight other.weight
13. sort => next_permutation(.begni(), .end());
14. operator<  weight < other.weight => ascending order
15. operator<  weight < other.weight => min_heap
