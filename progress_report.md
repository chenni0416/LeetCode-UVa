# 🚀 LeetCode 75 Mastery Tracker

### 📋 Status Legend & Notes Guide
| Status | Tag (in .cpp) | Description |
| :--- | :--- | :--- |
| ✅ **Solved** | *(None)* | Perfectly solved. Default state. |
| 🎯 **Active** | `// Status: ACTIVE` | Planning to solve today or currently working on it. |
| 🔄 **Review** | `// Status: REVIEW` | Solved but need practice. |
| 🛑 **Stuck**  | `// Status: STUCK` | Couldn't solve, needs deep study. |
| ⬜ **Pending**| *(No File)* | Not started yet. |

> 💡 **How to add notes**: Add `// Note: Your text here` at the top of your `.cpp` files. Multiple lines are supported!

---

## 🏆 Global Progress
> **Overall:** `[██████░░░░░░░░░░░░░░]` **29%** (22/75)
---

### 📁 Arrays
> **Progress:** `[███░░░░░░░]` **33%** (1/3)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Majority Element | - |
| ✅ **Solved** | Product of Array Except Self | - |
| ⬜ Pending | First Missing Positive | - |


### 📁 Strings
> **Progress:** `[█████░░░░░]` **50%** (1/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Is Subsequence | - |
| ✅ **Solved** | Reverse Words in a String | - |


### 📁 Bit Manipulation
> **Progress:** `[░░░░░░░░░░]` **0%** (0/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Counting Bits | - |
| ⬜ Pending | Single Number III | - |


### 📁 Hash Tables
> **Progress:** `[██████████]` **100%** (2/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Group Anagrams | - |
| ✅ **Solved** | Longest Consecutive Sequence | - |


### 📁 Two Pointers
> **Progress:** `[███████░░░]` **66%** (2/3)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Container With Most Water | - |
| 🛑 **Stuck** | 3Sum | 💡 So difficult |
| ⬜ Pending | Trapping Rain Water | - |


### 📁 Prefix Sum
> **Progress:** `[██████████]` **100%** (1/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Subarray Sum Equals K | - |


### 📁 Sliding Window - Fixed Size
> **Progress:** `[██████████]` **100%** (2/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Find All Anagrams in a String | - |
| ✅ **Solved** | Permutation in String | - |


### 📁 Sliding Window - Dynamic Size
> **Progress:** `[█████░░░░░]` **50%** (1/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Longest Substring Without Repeating Characters | - |
| ⬜ Pending | Minimum Window Substring | - |


### 📁 Kadane's Algorithm
> **Progress:** `[██████████]` **100%** (1/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Maximum Subarray | - |


### 📁 Matrix (2D Array)
> **Progress:** `[██████████]` **100%** (2/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Spiral Matrix | - |
| ✅ **Solved** | Rotate Image | - |


### 📁 Linked List
> **Progress:** `[██████████]` **100%** (3/3)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Remove Nth Node From End of List | - |
| ✅ **Solved** | Swap Nodes in Pairs | - |
| ✅ **Solved** | Add Two Numbers | - |


### 📁 LinkedList In-place Reversal
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Reverse Nodes in k-Group | - |


### 📁 Fast and Slow Pointers
> **Progress:** `[██████████]` **100%** (1/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Linked List Cycle II | - |


### 📁 Stacks
> **Progress:** `[█████░░░░░]` **50%** (1/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Valid Parentheses | - |
| ✅ **Solved** | Min Stack | - |


### 📁 Monotonic Stack
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Largest Rectangle in Histogram | - |


### 📁 Monotonic Queue
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Sliding Window Maximum | - |


### 📁 QuickSort - QuickSelect
> **Progress:** `[██████████]` **100%** (1/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Sort Colors | - |


### 📁 Binary Search
> **Progress:** `[███████░░░]` **66%** (2/3)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Find First and Last Position of Element in Sorted Array | - |
| ✅ **Solved** | Search in Rotated Sorted Array | - |
| ⬜ Pending | Median of Two Sorted Arrays | - |


### 📁 Backtracking
> **Progress:** `[██████████]` **100%** (2/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ✅ **Solved** | Permutations | - |
| ✅ **Solved** | Subsets | - |


### 📁 Tree Traversal - Level Order
> **Progress:** `[░░░░░░░░░░]` **0%** (0/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Binary Tree Level Order Traversal | - |
| ⬜ Pending | Binary Tree Right Side View | - |


### 📁 Tree Traversal - Pre Order
> **Progress:** `[░░░░░░░░░░]` **0%** (0/3)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Binary Tree Preorder Traversal | - |
| ⬜ Pending | Path Sum III | - |
| ⬜ Pending | Serialize and Deserialize Binary Tree | - |


### 📁 Tree Traversal - In Order
> **Progress:** `[░░░░░░░░░░]` **0%** (0/3)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Binary Tree Inorder Traversal | - |
| ⬜ Pending | Validate Binary Search Tree | - |
| ⬜ Pending | Kth Smallest Element in a BST | - |


### 📁 Tree Traversal - Post-Order
> **Progress:** `[░░░░░░░░░░]` **0%** (0/3)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Binary Tree Postorder Traversal | - |
| ⬜ Pending | Lowest Common Ancestor of a Binary Tree | - |
| ⬜ Pending | Binary Tree Maximum Path Sum | - |


### 📁 BST - Ordered Set
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | My Calendar I | - |


### 📁 Tries
> **Progress:** `[░░░░░░░░░░]` **0%** (0/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Implement Trie (Prefix Tree) | - |
| ⬜ Pending | Word Search II | - |


### 📁 Two Heaps
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Find Median from Data Stream | - |


### 📁 Top K Elements
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Top K Frequent Elements | - |


### 📁 Intervals
> **Progress:** `[░░░░░░░░░░]` **0%** (0/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Merge Intervals | - |
| ⬜ Pending | Non-overlapping Intervals | - |


### 📁 K-Way Merge
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Merge k Sorted Lists | - |


### 📁 Data Structure Design
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | LRU Cache | - |


### 📁 Greedy
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Jump Game II | - |


### 📁 Depth First Search (DFS)
> **Progress:** `[░░░░░░░░░░]` **0%** (0/3)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Number of Islands | - |
| ⬜ Pending | Clone Graph | - |
| ⬜ Pending | Is Graph Bipartite? | - |


### 📁 Breadth First Search (BFS)
> **Progress:** `[░░░░░░░░░░]` **0%** (0/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Rotting Oranges | - |
| ⬜ Pending | Word Ladder | - |


### 📁 Topological Sort
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Course Schedule II | - |


### 📁 Union Find
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Number of Provinces | - |


### 📁 Minimum Spanning Tree
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Min Cost to Connect All Points | - |


### 📁 Shortest Path
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Cheapest Flights Within K Stops | - |


### 📁 1-D DP
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | House Robber II | - |


### 📁 0-1 Knapsack
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Partition Equal Subset Sum | - |


### 📁 Unbounded Knapsack
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Coin Change | - |


### 📁 Longest Increasing Subsequence (LIS)
> **Progress:** `[░░░░░░░░░░]` **0%** (0/1)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Longest Increasing Subsequence | - |


### 📁 2D Grid DP
> **Progress:** `[░░░░░░░░░░]` **0%** (0/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Minimum Path Sum | - |
| ⬜ Pending | Longest Increasing Path in a Matrix | - |


### 📁 String DP
> **Progress:** `[░░░░░░░░░░]` **0%** (0/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Longest Common Subsequence | - |
| ⬜ Pending | Word Break | - |


### 📁 Maths - Geometry
> **Progress:** `[░░░░░░░░░░]` **0%** (0/2)

| Status | Problem | Notes / Takeaways |
| :--- | :--- | :--- |
| ⬜ Pending | Reverse Integer | - |
| ⬜ Pending | Max Points on a Line | - |

